...
A cross-platform C++ tool that repairs broken clipboard managers — focused on fixing the **frozen/unresponsive Win+V** panel on Windows 10/11.
...
Go to [Releases](../../releases) and download the binary for your OS. Run it.
...
> **Windows users:** Right-click the `.exe` → *Run as administrator* for full repairs.
...
The Win+V clipboard panel freezes (opens but won't scroll, click, or respond) when `TextInputHost.exe` — the process that hosts the clipboard UI — gets stuck. Simply restarting the clipboard service doesn't help because the frozen UI process is still running on top of it.
...
2. **Settings → System → Clipboard** — toggle *Clipboard history* off, wait 5 sec, toggle on
...
**Manual fix:**
1. **Settings → Apps → [Your keyboard] → Storage → Clear Cache**
2. Force-stop the keyboard app, reopen it
3. Samsung users: Samsung Keyboard Settings → Clipboard → clear history
...
- Restart your device
- Check **Settings → Privacy & Security → Paste from Other Apps**
